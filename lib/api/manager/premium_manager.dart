...
class PremiumManager {
  ...
  Future<void> updateGitHubSponsorPremium() async {
    ...
    final userToken = await repoManager.getStringNullable(
    ...
    final fileRes = await http.get(
      Uri.parse(
        'https://api.github.com/repos/ViscousPot/sponsors/contents/sponsors.txt',
      ),
      headers: {
        'Authorization': 'token $gitHubSponsorsToken',
    ...
    await repoManager.setBool(
      StorageKey.repoman_hasGHSponsorPremium,
      isSponsor,
    );
    hasPremiumNotifier.value = await _readPremiumStatus();
  }
  ...
  Future<void> cullNonPremium() async {
    final repomanReponames = await repoManager.getStringList(
    ...
    if (repomanReponames.length > 1) {
      ...
      await repoManager.setStringList(StorageKey.repoman_repoNames, [
        repomanReponames[0],
      ]);
    }
  }
