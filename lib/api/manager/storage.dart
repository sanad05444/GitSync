...
enum StorageKey<T> {
  ...
  repoman_hasGHSponsorPremium<bool>(
    name: "hasGHSponsorPremium",
    defaultValue: false,
  ),
  repoman_repoIndex<int>(name: "repoIndex", defaultValue: 0),
  repoman_tileSyncIndex<int>(name: "tileSyncIndex", defaultValue: 0),
  repoman_tileManualSyncIndex<int>(
    name: "tileManualSyncIndex",
    defaultValue: 0,
  ),
  ...
  repoman_repoNames<List<String>>(
    name: "repoNames",
    defaultValue: <String>["main"],
  ),
  repoman_locks<List<String>>(name: "locks", defaultValue: <String>[]),
  repoman_showGithubAppRedirectDisclosure<bool>(
    name: "showGithubAppRedirectDisclosure",
    defaultValue: true,
  ),
  repoman_reportIssueToken<String?>(
    name: "reportIssueToken",
    defaultValue: null,
  ),
  ...
  setman_syncMessageTimeFormat<String>(
    name: "syncMessageTimeFormat",
    defaultValue: syncMessageTimeFormat,
  ),
  setman_remote<String>(name: "remote", defaultValue: "origin"),
  setman_syncMessageEnabled<bool>(
    name: "syncMessageEnabled",
    defaultValue: false,
  ),
  ...
  setman_applicationObserverExpanded<bool>(
    name: "applicationObserverExpanded",
    defaultValue: true,
  ),
  setman_scheduledSyncSettingsExpanded<bool>(
    name: "scheduledSyncSettingsExpanded",
    defaultValue: false,
  ),
  setman_schedule<String>(name: "schedule", defaultValue: "never|0"),
  setman_otherSyncSettingsExpanded<bool>(
    name: "otherSyncSettingsExpanded",
    defaultValue: false,
  ),
  ...
  setman_gitCommitSigningKey<String?>(
    name: "gitCommitSigningKey",
    defaultValue: null,
  ),
  setman_gitCommitSigningPassphrase<String?>(
    name: "gitCommitSigningPassphrase",
    defaultValue: null,
  );
...
class Storage<T extends StorageKey> {
  ...
  Future<String?> getStringNullable(StorageKey<String?> key) async =>
      _get<String?>(key);
  Future<int> getInt(StorageKey<int> key) async => _get<int>(key);
  Future<List<String>> getStringList(StorageKey<List<String>> key) async =>
      _get<List<String>>(key);

  Future<void> setBool(StorageKey<bool> key, bool value) async =>
      _set<bool>(key, value);
  Future<void> setString(StorageKey<String> key, String value) async =>
      _set<String>(key, value);
  Future<void> setStringNullable(
    StorageKey<String?> key,
    String? value,
  ) async => _set<String?>(key, value);
  Future<void> setInt(StorageKey<int> key, int value) async =>
      _set<int>(key, value);
  Future<void> setStringList(
    StorageKey<List<String>> key,
    List<String> value,
  ) async => _set<List<String>>(key, value);
  ...
  Future<void> _set<N>(StorageKey<N> key, N value) async {
    ...
    if (N == getType<List<String>?>() || N == getType<List<String>>()) {
      await storage.write(
        key: getKeyName(key),
        value: value == null ? "" : (value as List<String>).join(","),
      );
      return;
    }
